...
def polynomial_mmd_averages(codes_g, codes_r, n_subsets=50, subset_size=1000,
    ...
    with tqdm(range(n_subsets), desc='MMD', file=output) as bar:
        for i in bar:
            g = codes_g[choice(len(codes_g), subset_size, replace=True)] #changed replace from False to True
            r = codes_r[choice(len(codes_r), subset_size, replace=True)] #changed replace from False to True
